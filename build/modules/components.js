...
function getComponents(config) {
  ...
  if(!config.cache || cache.getComponents.expiry < Date.now()) {
    ...
    return new Promise(function(resolve, reject) {
      glob(path.join(config.src, '**/component.yaml'), function (er, files) {

        var queue = [];

        files.forEach(function(filename) {
          queue.push(getComponent(path.dirname(filename).replace(config.src, ''), config));
        });
 ...
 * @return {Promise} Promise which resolves with the component data
 */
function resolveComponentDependencies(component, graph, components, config) {
  return new Promise(function(resolve, reject) {
      ...
      component.dependencies.forEach(function(dependency) {
        ...
        if(!graph.hasNode(dependency)) {
          ...
          promises.push(new Promise(function(resolve, reject) {
            getComponent(dependency, config)
...
function populateTree(componentsTree, config) {
  var promises = [];

  componentsTree.forEach(function(componentId) {
    promises.push(getComponent(componentId, config));
  });
